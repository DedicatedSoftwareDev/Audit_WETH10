...
contract WETH10 is IWETH10 {
    ...
    /// @dev Calculate the DOMAIN_SEPARATOR.// @notice chaidId is used for preventing reply attack
    ...
    function depositTo(address to) external override payable { //@audit no check address this
        ...
        balanceOf[to] += msg.value; // @audit overflow if user 
    ...
    function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 value, bytes calldata data) external override returns (bool) {//@audit reentrancy passible
        require(token == address(this), "WETH: flash mint only WETH10");
